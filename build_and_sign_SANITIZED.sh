...
# OTTOMAT3D Complete Build & Sign Script (Sanitized for Portfolio)
# Builds with PyInstaller, applies fixes, and signs in one go
# Usage: ./build_and_sign_SANITIZED.sh [architecture]
# Example: ./build_and_sign_SANITIZED.sh x86_64
# Example: ./build_and_sign_SANITIZED.sh arm64

set -e  # Exit on any error

# Configuration - REPLACE WITH YOUR INFORMATION
DEVELOPER_ID="Developer ID Application: [YOUR_NAME] ([YOUR_TEAM_ID])"
SITE_PACKAGES_PATH="[YOUR_PYTHON_SITE_PACKAGES_PATH]"
# Example: /Users/yourname/Desktop/Projects/ottomat3d/src/_internal/python-3.13-mac/lib/python3.13/site-packages
...
prepare_cross_arch_packages() {
    ...
    if [[ "$arch" == "x86_64" ]]; then
        ...
        # Move architecture-specific packages to backup
        ...
        for package in "${arch_packages[@]}"; do
            ...
            # Backup associated .dist-info directories
            ...
            # Handle Pillow's special case (PIL vs pillow naming)
        ...
        if [[ "$install_success" == true ]]; then
            ...
            if [[ "$verification_failed" == true ]]; then
                log_error "Package verification failed - ARM64 binaries found"
                rm -rf "$temp_packages"
                exit 1
            fi
            ...
            # Verify modified packages are preserved (if applicable)
            if [[ -d "$site_packages/bambulabs_api" ]]; then
                ...
                if [[ -f "$mqtt_client_file" ]] && grep -q "def disconnect" "$mqtt_client_file"; then
                    ...
                    log_warning "⚠️  bambulabs_api exists but may not have custom fixes"
                fi
            fi
        else
            log_error "Failed to download x86_64 packages"
            rm -rf "$temp_packages"
            exit 1
        fi
    fi
...
run_pyinstaller() {
    ...
    log_info "Site-packages path: $SITE_PACKAGES_PATH"
    
    # Run PyInstaller - all dependencies configured in the .spec file
...
apply_terminal_wrapper_fix() {
    ...
    # Copy the terminal wrapper
    log_info "Copying terminal wrapper..."
...
test_app_launch() {
    ...
    log_warning "The app will open in Terminal. Close it after verifying it works."
...
main() {
    ...
    # Always restore packages after build
    restore_packages
    ...
    if verify_signing "$app_path"; then
        ...
        log_error "Signing verification failed. Check your Developer ID certificate."
        exit 1
    fi
}
